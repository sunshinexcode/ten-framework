    ...
    async def on_init(self, ten_env: AsyncTenEnv) -> None:
        try:
            ...
            if not config_json_str or config_json_str.strip() == "{}":
                raise ValueError(
                    "Configuration is empty. Required parameters are missing."
                )

            self.config = TencentTTSConfig.model_validate_json(config_json_str)
            ...
            ten_env.log_info("TencentTTS client initialized successfully")
        ...
        try:
            ...
            async for audio_chunk, event_status in data:
                ...
                if event_status == EVENT_TTS_RESPONSE:
                    ...
                    self.ten_env.log_info("Received TTS_END event from Tencent TTS")
                    # Send TTS audio end event
                    if self.sent_ts and t.text_input_end and self.current_request_id:
                        request_event_interval = int(
                            (datetime.now() - self.sent_ts).total_seconds() * 1000
                        )
                        duration_ms = self._calculate_audio_duration_ms()
                    ...
                    self.ten_env.log_error(f"TTS error received: {error_msg}")
                    if self.current_request_id:
                        await self.send_tts_error(
                            self.current_request_id,
                            ModuleError(
                    ...
                    self.ten_env.log_info("TTS stream flushed")
                    if self.sent_ts and self.current_request_id:
                        request_event_interval = int(
                            (datetime.now() - self.sent_ts).total_seconds() * 1000
                        )
            ...
            self.ten_env.log_error(
                f"TencentTTSTaskFailedException in request_tts: {e.error_msg}. text: {t.text}"
            )
            await self.send_tts_error(
                self.current_request_id,
                ModuleError(
