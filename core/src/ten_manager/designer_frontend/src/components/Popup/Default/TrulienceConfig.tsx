...
export const TrulienceConfigWidgetContent = (_props: { widget: IWidget }) => {
  ...
  const schema:[string, z.ZodType][] = [
    ["enabled", z.boolean().optional().superRefine(
      fieldConfig({
        label: t("trulienceConfig.enabled")
      })
    )],
    ["trulienceAvatarId", z.string().optional().superRefine(
      fieldConfig({
        label: t("trulienceConfig.trulienceAvatarId")
      })
    )],
    ["trulienceAvatarToken", z.string().optional().superRefine(
      fieldConfig({
    ...
    <div className="flex flex-col gap-2 h-full w-full">
      <>
        <AutoForm
          ...
          schema={
            new ZodProvider(
              z.object(Object.fromEntries(schema))
            )
          }
        >
            <Button 
