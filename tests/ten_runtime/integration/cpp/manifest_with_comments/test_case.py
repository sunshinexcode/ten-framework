"""
Test manifest_with_comments_app.
"""
...
def test_manifest_with_comments_app():
    ...
    app_dir_name = "manifest_with_comments_app"
    ...
    if sys.platform == "win32":
        my_env["PATH"] = (
            os.path.join(
                base_path,
                "manifest_with_comments_app/ten_packages/system/ten_runtime/lib",
            )
            + ";"
            + my_env["PATH"]
        )
        server_cmd = os.path.join(
            base_path,
            "manifest_with_comments_app/bin/manifest_with_comments_app.exe",
        )
        client_cmd = os.path.join(
            base_path, "manifest_with_comments_app_client.exe"
        )
        ...
        server_cmd = os.path.join(
            base_path,
            "manifest_with_comments_app/bin/manifest_with_comments_app",
        )
        client_cmd = os.path.join(
            base_path, "manifest_with_comments_app_client"
        )
    else:
        # client depends on some libraries in the TEN app.
        ...
        if (
            ...
            libasan_path = os.path.join(
                base_path,
                (
                    "manifest_with_comments_app/ten_packages/system/"
    ...
    if not is_started:
        print("The manifest_with_comments_app is not started after 30 seconds.")

        server.kill()
        exit_code = server.wait()
        print("The exit code of manifest_with_comments_app: ", exit_code)
